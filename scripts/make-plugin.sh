...
# A directory that grants entry and writes but not listing yields no output
# here, and no output is exactly how an empty directory looks — so an
# uninspectable destination would pass this guard and be wiped below.
if ! dest_entries="$(ls -A "$dest_abs" 2>/dev/null)"; then
  echo "error: cannot list $dest_abs — refusing to rebuild into a destination that cannot be inspected" >&2
  exit 1
fi
if [ -n "$dest_entries" ] && [ ! -f "$dest_abs/$MARKER" ]; then
